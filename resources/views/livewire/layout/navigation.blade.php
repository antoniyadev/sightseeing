...
<nav x-data="{ open: false }" class="bg-white border-b border-gray-100 dark:bg-gray-800 dark:border-gray-700">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="flex items-center shrink-0">
                    <a class="flex" href="{{ route('sights.index') }}" wire:navigate>
                        <x-application-logo class="block w-auto text-gray-800 fill-current h-9 dark:text-gray-200" />
                        <span class="px-2 text-xl">Sighty</span>
                    </a>
                </div>

                <!-- Navigation Links -->
                {{-- <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    <x-nav-link :href="route('sights.index')" :active="request()->routeIs('sights')" wire:navigate>
                        {{ __('Sights') }}
                    </x-nav-link>
                </div> --}}
            </div>
            ...
            @else
            <div class="flex pt-4 pb-1 space-x-4 border-t border-gray-200 dark:border-gray-600">
                   <a wire:navigate href="/login">Login</a>
                    <a wire:navigate href="/register">Register</a>
            @endauth
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            <x-responsive-nav-link :href="route('sights.index')" :active="request()->routeIs('sights')" wire:navigate>
                {{ __('Sights') }}
